namespace KanbanApp.Core.Model
{
	public class Column
	{
		...
		private Column(Guid id, string name, Guid boardId, int order)
		{
			Id = id;
			Name = name;
			BoardId = boardId;
		...
		public static (Column ColumnKanban, string Error) Create(Guid id, string name, Guid boardId, int order)
		{
			...
			var column = new Column(id, name, boardId, order);
			return (column, error);
		}
	}
}
