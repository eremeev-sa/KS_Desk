...
public class ColumnRepository : IColumnsRepository
{
	...
	public ColumnRepository(KanbanAppDbContext context)
	{
		_context = context;
	}
	...
	public async Task<List<Column>> Get()
	{
		...
		var columns = columnsEntities
			.Select(c => Column.Create(c.Id, c.Name, c.BoardId, c.Order).ColumnKanban)  
			.ToList();  
	...
	public async Task<List<Column>> GetByBoardId(Guid boardId)
	{
		...
		var columns = columnEntities
			.Select(c => Column.Create(c.Id, c.Name, c.BoardId, c.Order).ColumnKanban)  // Преобразование
			.ToList();  
